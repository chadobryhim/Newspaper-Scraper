...
router.get('/scrape', function(req, res) {
  ...
  request('https://thebakerorange.com/category/news/', function(error, response, html) {
    ...
    // Now, grab every everything with a class of "searchheadline" with each "article" tag
    $('article .searchheadline').each(function(i, element) {
        ...
        result.link = 'http://www.thebakerorange.com' + $(this).children('header').children('h1').children('a').attr('href').trim();

        // Collect the Article Summary (contained in the next "p" inside of "this")
        ...
        if(result.title !== "" &&  result.summary !== ""){
          ...
          // BUT we must also check within each scrape since the Orange has duplicate articles...
